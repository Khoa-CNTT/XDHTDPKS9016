<template>
  <div
    v-if="hotel"
    class="p-8 bg-gradient-to-r from-gray-100 to-blue-100"
  >
    <div class="container mx-auto px-8 grid grid-cols-1 md:grid-cols-3 gap-6 py-2">
      ...
      <div class="bg-white p-6 rounded-lg shadow-lg">
        ...
        <div class="space-y-4">
          ...
          <div
            v-for="(hotel, index) in hotelList.slice(0, showAllHotels ? hotelList.length : 4)"
    ...
    <div
      class="container mx-auto px-8 pt-4"
      v-if="hotel?.roomTypes?.length"
    >
    ...
    >
      ...
      <div
        class="w-full border text-sm md:text-base flex flex-wrap gap-6 p-6 bg-gray-100 rounded-lg"
      >
        <div
          v-for="(room, index) in hotel.roomTypes"
          :key="index"
        ...
        >
          <img
            :src="
              room.room_image
                ? 'http://localhost:8080' + room.room_image
                : 'http://localhost:8080/api/v1/images/view/b3a94d55-e4a4-42cf-8e9d-9e785d4488a7_room-9.jpg'
            "
            alt="Hotel Image"
    ...
    <div class="container mx-auto px-8 mb-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="flex items-end">
          <button
            @click="filterRooms"
            class="inline-flex items-center gap-1 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-semibold text-sm px-4 py-2 rounded-full shadow-md transition-transform duration-150 active:scale-95 active:shadow-inner focus:outline-none focus:ring-2 focus:ring-purple-300"
          >
            <svg
    ...
    <div class="bg-gradient-to-r from-gray-100 to-blue-100 py-1">
      <div class="container mx-auto px-4 md:px-8">
        <div
          class="flex flex-col md:flex-row gap-8 border border-gray-300 p-6 rounded-xl shadow-lg"
        >
          <!-- Cột trái - Hình ảnh và mô tả khách sạn -->
          <div class="md:w-1/3 bg-white rounded-xl shadow-md p-4">
            <img
              :src="`localhost:8080${hotel.image}`"
              alt="Main room image"
              class="rounded-lg w-full"
            />
            <div class="flex gap-2 mt-3">
              <img
                v-for="(roomType, i) in hotel.roomTypes.slice(0, 2)"
                :key="i"
                :src="`http://localhost:8080${roomType.room_image}`"
            ...
            >
              ...
              >
                ...
                <div class="sm:w-1/3 p-4 border-b sm:border-b-0 sm:border-r border-gray-300">
                  ...
                  <div
                    v-if="!roomType.rooms || roomType.rooms.length === 0"
                  ...
                  >
                    ...
                    >
                      ...
                      <div class="text-red-700 text-center mb-2 sm:mb-0 sm:text-right sm:mr-4">
                        <div
                          class="text-sm bg-red-100 text-red-600 font-semibold px-2 py-1 rounded"
                        >
                          Giá mỗi đêm
                        </div>
                      ...
                      <div>
                        <button
                          @click="checkLogin(room, roomType)"
                          class="bg-blue-600 text-white px-4 py-2 rounded w-full"
                        >
                          Đặt ngay
                        </button>
                        <div class="text-green-600 text-sm font-semibold text-center mt-1">
                          Còn phòng
                        </div>
                      </div>
                    </div>
...
const comments = ref([])
const checkLogin = (room, roomType) => {
